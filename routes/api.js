...
var creatorList = ['Ardyy','Ardyy','Ardyy','Ardyy', 'Ardyy','Ardyy','Ardyy'];
...
loghandler = {
    notparam: {
        ...
        getApikey: 'apikey gratis https://wa.me/6281344309802'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
    ...
    invalidKey: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'apikey gratis https://wa.me/6281344309802'
    },
    invalidlink: {
        status: false,
        creator: `${creator}`,
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'apx7api') return res.json(loghandler.invalidKey)
...
router.get('/textmaker/ephoto', async (req, res, next) => {
	...
	if(apikeyInput != 'apx7api') return res.json(loghandler.invalidKey)
        ...
        if (theme != 'graffiti' && theme != 'tes') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
        ...
        if (theme == 'graffiti') {
            ...
            request.post({
                ...
                body: `text-0=${text}&text-1=${text2}&submit=GO`,
                }, (e,r,b) => {
                    if (!e) {
                        $ = cheerio.load(b)
...
router.get('/imgmanipulation', async (req, res, next) => {
        var var apikeyInput = req.query.apikey,
            url = req.query.url
    ...
    if (!url) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter url"})
        	...
        	if (!theme) return res.json(loghandler.nottheme)
        if (theme != '3dline' && theme != 'tes') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
        ...
        if (theme == '3dline') {
        	...
        	if (!url) return res.json(loghandler.nottext2)
        if (!url) return res.json(loghandler.nottext3)
            request.post({
                url: "https://photooxy.com/other-design/make-google-suggestion-photos-238.html",
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                },
                body: `text_1=${url}&text_2=${url}&text_3=${url}&login=OK`,
                }, (e,r,b) => {
                    if (!e) {
                        $ = cheerio.load(b)
                        $(".thumbnail").find("img").each(function() {
