...
router.get('/fps', async (req, res, next) => {
	...
	if(apikeyInput != 'apx7api') return res.json(loghandler.invalidKey)
        if (!theme) return res.json(loghandler.nottheme)
        if (theme != 'fpsgame' && theme != 'tes') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
        if (!url) return res.json(loghandler.noturl)
        ...
        if (theme == 'fpsgame') {
            ...
            try {
            request.post({
                url: "https://photooxy.com/game-effects/cool-circle-guns-wallpapers-141.html",
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                },
                body: `image_0=${url}&text_2=${text}&text_3=${text2}&login=OK`,
                }, (e,r,b) => {
                    if (!e) {
                        $ = cheerio.load(b)
                        $(".thumbnail").find("img").each(function() {
